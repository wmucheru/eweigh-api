...
class Auth_model extends CI_Model{
    ...
    function phoneExists($mobile){
        $exists = $this->db->get_where('aauth_users', array('mobile'=>$mobile));
        return $exists->num_rows() > 0;
    }

    # Return user info, including user group
    # TODO: Merge with get_user_info function
    function get_api_user_info($email){
        $this->db->select('
            au.id, au.names, au.email, au.mobile, au.photo,
        ...
        $this->db->join('aauth_groups ag', 'ag.id=autg.group_id', 'left');

        $this->db->where('au.email', $email);

        $user = $this->db->get();

        return $user->num_rows() > 0 ? $user->row() : array();
    }
    ...
    function update_member_group($user_id, $group_par){
        ...
        $this->db->delete('aauth_user_to_group', $userObj);
        
        return $this->db->insert('aauth_user_to_group', $userObj);
    }
